...
import { examCheckStudentMark } from '../crud/requests';
...
const StudentExamResponse : React.FC = () : JSX.Element => {
  ...
  const { mutate, isLoading } = useMutation({
    mutationFn: () => examCheckStudentMark(exam_controle_student_id, student_ball),
    onSuccess: async (res) => {
        refetch()
            ...
            <div className="grid grid-cols-12 gap-4">
                <div className="lg:col-span-4 col-span-12">
                    <div className="shadow-sm rounded-md p-4 border-solid border-2 border-slate-100">
                        ...
                        {
                            data?.data?.examControl?.duration ? 
                ...
                <div className="lg:col-span-8 col-span-12">
                    <div className="shadow-sm rounded-md p-4 border-solid border-2 border-slate-100">
                        {
                            data?.data?.type === 2 ?  // test shaklida
                                <div className="flex items-center mb-4">
...

                        {
                            ...
                            <div>
                                <div className="flex items-start mb-4">
                                    ...
                                    <div>
                                        <div ref={divRef} className={`w-full ${is_show_more ? "": 'line-clamp-5'}`} dangerouslySetInnerHTML={{ __html: data?.data?.answer_text ?? "" }} />
                                        <Button type="text" onClick={() => setis_show_more(!is_show_more)} className='mt-3'>...{is_show_more ? t('See less') : t('See more')}</Button>                              
                                    </div>
                                </div>
                                <div className="flex items-center mb-4">
                        ...
                        <div className="flex items-center my-4"> 
                            ...
                            <p>{data?.data?.user_status === 2 ? data?.data?.student_ball : data?.data?.user_status === 0 ? <Tag color="error" >{t('The student has not passed the exam yet')}</Tag> : <Tag color="warning">Talaba baholanmagan!</Tag>}</p>
                        </div>
                        {
                            data?.data?.type === 1 ? // yozma yoki fayl shaklida bolsa baholash
                            <div className="">
                                <Form
                                    form={form}
                                    layout="vertical"
                    ...
                    { 
                      data?.data?.type === 2 && data?.data?.user_status !== 0 ?   
