...
const DocDecreeView: React.FC = (): JSX.Element => {
    ...
    <div className="">
      <HeaderExtraLayout
        breadCrumbData={[
          ...
          { name: "Farmoyishni ko'rish", path: '/doc-decree/view/:id' }
        ]}
        title={t("Farmoyish")}
        isBack={true}
        btn={
          <div>
            {
              checkPermission("document-decree_create") && !data?.data?.status ?
                <Button
                  type="primary"
                  className="px-5 ml-2"
                  onClick={() => navigate(`/doc-decree/update/${data?.data?.id}`)}
                >
                  Tahrirlash
                </Button> : ""
            }
