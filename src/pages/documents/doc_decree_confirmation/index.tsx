...
const DocDecreeConfirm = () => {
  ...
  const { data, refetch, isFetching: isLoading } = useGetAllData({
    queryKey: ["document-decrees/confirm", urlValue.perPage, urlValue.currentPage, urlValue.filter_like?.sort, urlValue.filter_like?.type ],
    url: `document-decrees/confirm?expand=user,user.profile,body&filter=${JSON.stringify({status: 1, type: urlValue.filter_like?.type})}`,
  ...
  const columns: ColumnsType<any> = React.useMemo(() => [
    ...
    },
    {
      ...
      render: (i, e) => <Actions
        ...
        onClickView={() => navigate(`/doc-decree/confirm/${e?.id}`)}
...
export default DocDecreeConfirm;
