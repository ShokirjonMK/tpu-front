...
import { docNoticeConfirm } from "./crud/request";

const DocNoticeConfirm = () => {
  ...
  const { data, refetch, isFetching: isLoading } = useGetAllData({
    queryKey: ["document-notifications/confirm", urlValue.perPage, urlValue.currentPage, urlValue.filter_like?.sort, urlValue.filter_like?.type ],
    url: `document-notifications/confirm?expand=user,user.profile,body&filter=${JSON.stringify({status: 1, type: urlValue.filter_like?.type})}`,
  ...
  const columns: ColumnsType<any> = React.useMemo(() => [
    ...
    },
    {
      ...
      render: (e: any) => {
        ...
        if (e?.type === 4) return <Tag color="green" >Imzolangan</Tag>
    ...
    },
    {
      ...
      render: (i, e) => <Actions
        ...
        onClickView={() => navigate(`/doc-notice/confirm/${e?.id}`)}
  ...
  return (
    <div className="">
      ...
      <Tag color={urlValue?.filter_like?.type === '2' ? "blue" : ""} onClick={() => writeToUrl({ name: "type", value: 2 })} className="cursor-pointer text-[14px] py-[2px] px-2" >Tasdiqlangan</Tag>
      <Tag color={urlValue?.filter_like?.type === '1' ? "blue" : ""} onClick={() => writeToUrl({ name: "type", value: 1 })} className="cursor-pointer text-[14px] py-[2px] px-2" >Qaytarilgan</Tag>
      <Tag color={urlValue?.filter_like?.type === '0' ? "blue" : ""} onClick={() => writeToUrl({ name: "type", value: "0" })} className="cursor-pointer text-[14px] py-[2px] px-2" >Kutilyotgan</Tag>
      <div className="">
        {/* <Row gutter={[12, 12]}>
...
export default DocNoticeConfirm;
