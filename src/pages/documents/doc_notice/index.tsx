...
const DocNotice = () => {
  ...
  const { data, refetch, isFetching: isLoading } = useGetAllData({
    queryKey: ["document-notifications", urlValue.perPage, urlValue.currentPage, urlValue.filter_like?.sort, urlValue.filter_like?.type, ...(Object.values(urlValue?.filter) ?? [])],
    url: `document-notifications?expand=user.profile,body,qrCode&filter=${JSON.stringify({...urlValue.filter, type: urlValue.filter_like?.type})}`,
  ...
  const columns: ColumnsType<any> = React.useMemo(() => [
    ...
    },
    {
      ...
      render: (i, e) => <Actions
        ...
        onClickView={() => navigate(`/doc-notice/view/${e?.id}`)}
        viewPermission={'document-notification_view'}
        editPermission={!e?.status ? 'document-notification_update' : ""}
        deletePermission={!e?.status ? 'document-notification_delete' : ""}
      />,
    },
  ], [data?.items]);
  ...
  return (
    <div className="">
      <HeaderExtraLayout
        ...
        title={t("Bildirgilar")}
        btn={
          checkPermission('document-notification_create') ? <Button type="primary" onClick={() => navigate("/doc-notice/create")} >Bildirgi yaratish</Button> : null
        }
      />
      <div className="p-3">
