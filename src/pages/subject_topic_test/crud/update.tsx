...
import FormTestOptionUI from './option_ui';
import useGetAllData from 'hooks/useGetAllData';
import checkPermission from 'utils/check_permission';
import DeleteData from 'components/deleteData';

const UpdateTopicTest = () => {
    ...
    <Spin spinning={isLoading && test_id != "0"} size="small">
      <div>
        <HeaderExtraLayout
          ...
          isBack={true}
          backUrl={`/subject/tests/${subject_id}/${topic_id}`}
          breadCrumbData={[
            { name: "Home", path: "/" },
            { name: `Subjects`, path: `/subjects` },
            { name: `Subject topics`, path: `/subjects/view/${subject_id}?user-block=topic-info` },
            { name: `Topic tests`, path: `/subject/tests/${subject_id}/${topic_id}` },
            { name: test_id == "0" ? "Test qo'shish" : "O'zgatirish", path: `/subject/tests/${subject_id}/${topic_id}` },
          ]}
          btn={
            <div className="flex items-center">
              <DeleteData
                ...
                url={"tests"}
                id={Number(test_id)}
                navigateUrl={`/subject/tests/${subject_id}/${topic_id}`}
              >
        ...
        <div className="pb-[24px]">
          <FormTestQuestionUI data={data?.data} refetch={refetch} isEdit={isEdit} setisEdit={setisEdit} />
          {
            optionsData?.items?.map((item, index) => (
              <FormTestOptionUI key={item?.id} data={item} refetch={refetchOptions} />
            ))
          }
          {
            options?.map((item, index) => (
              <FormTestOptionUI key={index} data={{} as ITestOption} refetch={refetchOptions} isNew={true} setOptions={setOptions} />
            ))
          }
...
export default UpdateTopicTest;