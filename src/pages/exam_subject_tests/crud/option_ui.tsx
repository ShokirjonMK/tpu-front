...
import { submitExamTestOption } from "./request";
...
const FormExamTestOptionUI = ({ data, refetch, isNew = false, setOptions}: { data: ITestOption; refetch: () => void, isNew?: boolean, setOptions?: Dispatch<Array<any>> }) => {
  ...
  const { mutate, isLoading } = useMutation({
    mutationFn: (newVals:any) => submitExamTestOption(option_id, {...newVals, question_id: test_id, file: fileList[0]?.originFileObj}),
    onSuccess: async (res) => {
      setisEdit(false)
      setisTrue(false)
      refetch()
...
export default FormExamTestOptionUI;
